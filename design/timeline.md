# Timeline — the standing ledger

**Purpose.** Act 4 lost control of its clock. Eleven months went past in one
sentence at the end of ch33, another eleven in the first line of ch34, three
more between ch36 and ch37, and a sixteen-month surplus arc was planned on top
of that. Twenty-nine of the thirty available months were spent as connective
tissue between emotional beats. This file exists so that does not happen again.

---

## THE RULE

**Time advances in days and weeks. A skip longer than one month requires a
reason on the page and must be entered in this file before it is written.**

Skips are not transitions. They are expenditure. The timeskip is a fixed budget
of thirty months and it has been almost entirely spent — every month written
past is a month of canon-quiet village life that cannot be recovered, because
the far side of it is the Akatsuki era and the story stops being about a
hospital.

Corollary: **do not close a chapter on a line like "eleven months later."** It
reads as elegance and costs a quarter of the act.

---

## FIXED AGES — check against these before writing any "X years ago"

| Event | Age | Source |
|---|---|---|
| Knee injury (field repair, wire) | ~7y | ch26 — "keep a seven-year-old walking" |
| Enters the trade / stock lists | ~7y | ch30, ch35 — "before he was eight" |
| Kohaku's courier runs begin | ~9y | world-bible — the drops |
| **Kohaku is the ELDER sister, 5–6 yrs older** | 12–13y when he is 7 | world-bible: already a working shinobi while he was in the Academy. **She is never a small child in any look-back.** |
| **Mother (Ruri Kaede) works the frames** | — | ch01 sc1. The family trade. **Not a medic, no hospital.** |
| **The execution. Mother (Ruri Kaede) and Kohaku both die.** | **13y6m** | world-bible age ledger, act-1 beats 21–23 |
| Crossing, Hot Water | 13y6m | act-2 Part A |
| Leaves Hot Water | 15y8m | age ledger |
| Sixteenth birthday, month 11 day 8 | 16y0m | ch17 sc1 |
| Konoha gate | 16y1m | ch22 |
| Konoha Crush | 16y7m | ch24–27 |

### Calendar months — use these, do not invent

His birthday is **month 11, day 8** (world-bible). Everything in Act 4 Part D is
dated off it.

| Chapter | Calendar |
|---|---|
| 37 | months 9 → 12. Birthday (8/11) falls inside; he learns of it five weeks late, in month 12 |
| 38 | month 12 → month 1 |
| 39 | month 1. Ashida's birthday 19/1; the party after |
| 40 | months 2 → 3. Urushibara dies 4/2 |
| 41 | months 3 → 4. Tsunade's birthday given as 2/8 — five months out, deliberately not acted on |
| 42 | month 4. About three weeks. Sakura, the marking scheme, the governor |
| 43 | months 4 → 5. About three weeks. Board on the 9th, Ibiki on the 14th, notice goes up the following Monday |
| 44 | month 5, the 2nd to the 22nd. Two weeks and a bit. Letter goes 2nd Thursday = the 21st |
| 45 | month 5, the 22nd to the 31st. Nine days. Sword started 31/5, 23:20 |
| 47 | month 8, the 1st to the 15th. Two weeks. Tenzō on the 6th; the reach 22:24 on the 15th |
| 48 | **One night.** 23:05 on the 15th to 16:00 on the 16th |
| 49 | month 8, the 17th to the 24th. Eight days. The rose delivered ~02:10 on the 25th |
| 50 | month 8 the 25th → month 9 the 30th. Five weeks. Contract refused 2/9; requisition filed 4/9; the count reaches nine on 30/9 |
| 51 | month 10, the 3rd to the 14th. Twelve days. The wood at 21:19 on the 14th |
| 52 | month 10, the 15th to the 31st. Seventeen days. Ame on the 24th; the book given 28/10, leaves for Shikkotsu 2/11 |
| **53** | month 11, the 4th to the 11th. Eight days. Instrument finished 4/11; sent for review 8/11; the hill 11/11. **ACT 4 ENDS.** |
| **54** | month 11, the 11th, evening into the 12th, ~01:00. One continuous night. The office, then her house. **ACT 5 OPENS.** Eight scenes, ~12,200 words. |
| **55** | month 11, the 12th, 07:15 to 08:10. One continuous morning, her house. Eight scenes, ~12,100 words. The seal seen; the rose found; the Ibiki deferral. |
| **56** | month 11, the 12th (Thu) to the 15th. Four days, the tower. Eight scenes, ~11,200 words. Shizune's assistant; the lost count; the form; the twelfth Friday missed; **Jiraiya at the gate**. |
| **57** | month 11, the 16th, 03:40 to ~06:00. One morning, the tower and its roof. Nine scenes, ~14,900 words. **POV Tsunade sc1–4**, Sazare sc5–9. The paper commissioned to Jiraiya; the word *sage*; the petrification re-explained; Tenzō named; **be there**. |
| **58** | month 11, the 17th to the 21st. Five days. Nine scenes, ~14,400 words. The exercise breaks; the chain designed; the twelfth Friday paid; **what are we**. POV Tsunade sc9. |
| **59** | month 11, the 21st to the 27th. Seven days. Eight scenes, ~15,300 words. The declaration; the emotional-state mechanic; the Shikkotsu review; Myōboku's four conditions; the ceiling event; **he moves into the tower**. POV Tsunade sc1. |
| **60** | month 11 the 28th to month 12 the 2nd. Five days. Nine scenes, ~16,000 words. *Love you too*; the blade carried open; two mountains; **the removal**; 0.11 grams; the lamp that went out. POV Tsunade sc1. |
| **61** | month 12, the 2nd to the 11th. Ten days. Nine scenes, ~19,300 words. The kiss he initiates; the governor; the toads go home; custody; the north store; **the first ordinary article**; exchange three; Tenzō named; **unknown**. |
| **62** | month 12, the 14th to the 17th. Four days. Nine scenes, ~17,000 words. Moving day; Naruto; Sakura's cover; the books split from the index; **Ōtaki Sue**; the ninth-year frame; **the spar set for the 20th** with an IOU on it. |
| **63** | month 12, the 16th to the 19th. Four days' training. Eight scenes, ~14,500 words. The mountains' wager; the manual come back broken; the conversion table; range is a price; **placed articles are free**; the scalpel sheet; the eyes alluded to; *believe in me*. POV Tsunade sc5, 6, 8. |
| **64** | month 12, the 20th. One day, 09:30 to ~20:30. Nine scenes, ~18,600 words. **The spar.** Slug sage mode; cannibalistic crystal; Naruto's leg; the thing in his abdomen; four hours in theatre two; the IOU paid. POV Sakura sc1, 4. |
| **65** | month 12 the 22nd to month 1 the 1st. Ten days. Eight scenes, ~16,400 words. The story travels; the box is finished; Jiraiya takes him on; the seal assessment; **the three questions spent**; the notes; **the IOU spent on a day out**. |
| **74** | month 3, the 16th to the 20th. Five days. Ten scenes, ~25,100 words. **The creed revised; three refusals.** The decision finding delivered and its last third corrected (*you trusted the impossibility of the harm*; the year is becoming somebody you'd trust with a stranger); **THE CREED READ AS AN ARTICLE — a governor, trigger CAPACITY**, laid at fifteen over *find out how it works*; the square said aloud whole (the spike 140 seconds early, the standing order, four of his own on the boards); Jiraiya's counter — *it doesn't work and you wrote the proof* (20/12), and **what stopped the twentieth being the square was THE CLOSE**: *staying small does not make the accident small, it makes the accident unskilled*; the promise named (Kohaku) and the absolution deliberately withheld; **Fukasaku's second answer — the mind with nothing left to defend, Myōboku's rite, the verdict named as a verdict**; **STRUCK 16/3, 07:11, witnessed**, replacement due Friday as a specification; the noon law suspended by its own author (*write what you're FOR, and make it something you can be caught failing at*); **Hinata commissions her own ambush** and the sword read under the Byakugan as *sourceless* and held undescribed; **Naruto's four steps and the refusal disclosed before it could be extracted** (*nobody ever told you there was one*) with the undertaking in the diary; **the thumb unfolded — WOOD** — and the fifth layer refused, permanently, in four minutes; **Tenzō holds the door, is asked at full price, and says no himself** — the button handed over; **chapter twelve read to the end and the interference pattern gone**, the count broken and condemned; **clause five** (*where the fabricator is unavailable, the article waits*); the treaty's first checkable condition (*the hat off me on a Tuesday, the lists up on the Wednesday*); **the amendment entered in all four books**. POV Jiraiya sc2; Tsunade sc3, 8; Hinata sc4; Tenzō sc7. |
| **73** | month 3, the 11th to the 15th. Five days. Nine scenes, ~12,100 words. **The border, the assessment, the deep end.** The boundary named (*he cannot cross what is not specified; she cannot ask except in formats*); the house prepared (the lamp bought Thursday, the glass untouched); **the combat assessment in her garden** — 3–1, his touch bought with clause eleven and eleven minutes of prepared ground; **both sealed papers say THE DEEP END**; the night — the room read and not read aloud, the scar kissed, **the baseline of four seconds ruled "day one, first exercise, complete"**, the interval of eighteen, *try again*; the uncounted hours and the deliberate gap in the ledger; **the glass washed, filled, set back — the house awake**; the new study opened (*method: graduated; the floor is not yet known*) and **THE DECISION FINDING for Jiraiya: the yes lives inside TRUST, not in harm.** POV Tsunade sc2, 5, 8. |
| **72** | month 2 the 28th to month 3 the second Tuesday. ~Ten days. Nine scenes, ~10,400 words. **Naruto's chapter.** First Naruto POV (four bowls, *a chair is a promise*); the meal (page sixty-one, amended; the rematch deferred to **midsummer, in the diary**); the first family dinner — the hotpot at the table's centre, **the grenade** ("you should marry somebody like Sazare" / "the production run was limited"), Fukasaku's vintage line, *the chair was yours before you sat in it*; Sakura's forecourt demolition and the conversion; **the collapse witnessed** (the kilns; *the street has a name*); the specification question (*a wound wearing a love's clothing*; "loved once, nearly killed me twice… both is the whole point"); Hinata's incident report (*the bridge must become standing*); **the audit upstream — the largest creditor**, Handa's zelkova chair, "family cooks," count at fifty-six, both columns; her upstream entry — Tanzaku, the bet, **the sixth face and its name**. POV Naruto sc1; Shizune sc3; Hinata sc7; Tsunade sc9. |
| **71** | month 2, the 20th to the 27th. Eight days. Eleven scenes, ~11,600 words. **The tease war, the watcher, the detonation.** The campaign file; the drift (eleven reconstructable instances); **Hinata** — the hardship clause, the rotation spec, the Byakugan constraint, and THE LAX MOMENT read in one pass; her POV (*what is seen and not offered is held*); **the clipboard principle** taught in exchange; the laxness audit and **the daylight treaty** (both names, filed under security remediation); the bath — *do my back*, the reversal, **the orange spine: "You've been reading that FILTH!?"**; the four-finding defense incl. chapter twelve (*before the towel*); Shizune hears the shout only (*not minuted*); two pages, three mercies, the suspended sentence (**he reads ch. 12 aloud, date at court's choosing**); **the reckoning — "Tell me it didn't work," the editorship, the monthly family dinner, "it worked"**; the archive shelf gains the manual; count at fifty-one; Hinata's consultation note. POV Tsunade sc2, 6, 7, 8, 9; Hinata sc4. |
| **70** | month 2, ~the 5th to the 19th. Two weeks. Eleven scenes, ~11,200 words. **The pining flips sides.** One-in-forty-two at the field; the noons colonized; clause four ("inform, not announce") and the clause-five threat; the dosimeter broken (muted copper read as progress, wrongly); *definition of beauty zzz* — the collection reaches fourteen; her self-diagnosis and the three-clause trap; Shizune: *"you made him learn to ask — your turn"*; Tenzō's kettle protocol; **THE FLAT COUNT** (41, eleven days) — *embezzled by my own projects*; Jiraiya's twelve-day reveal (*in a year, inside two days*); **the collision — Thursdays entire, the noons law written, exhibit three sealed (the spring's medical file, her name)**; six of fourteen certified; attachment thirteen signed with THE MARK (first time on paper in 400 years); **the trail to the curve (four seconds, filed)**; the bath — *he has a source and the source has chapters* — **shelved on purpose: second-to-last blow.** POV Tsunade sc4, 9, 11; Shizune sc5; Tenzō sc6. |
| **69** | month 1 the 19th to month 2 the second week. ~Three and a half weeks. Eleven scenes, ~14,600 words (redone at full density). **The Hime line.** Tachibana's rib and the binding survey (no box on the form); cleared at the field — **the fleet of ordinary draw-articles becomes the spring's camouflage**; the docket, the name approved (*rationale three withheld*); **attachment twelve — the lead-model appointment, his first returned tease, in writing, with a reference number**; the fitting (theory holds forty minutes, scheduled failure at her discretion); Sakura and the infantry (**retention: 100%**); the queue at 141, *functional leisure wear*, two million, the winter deficit twice; **the brush — the first maker's mark of his life**, next to the pinwheel. POV Tsunade sc3; Sakura sc5. |
| **68** | month 1, the 13th to the 17th. Five days, week one of the field. Seven scenes, ~8,200 words. **"I cannot locate the decision"** — the no as baseline, not failure; first Jiraiya POV; **the oil problem: Shikkotsu has no spring — and the battery is re-specified as the spring** (three keepers, Fukasaku's veto, *not a weapon, a syllabus*); the landing retired — the shirt, the chest question and the anatomy answer; **the hair: II-5 discarded, the shop opened an inch, fifty passes, the provenance handed over**; her ledger: *whatever he asks for next, the answer is yes*; week wrap — bruises eleven, count twenty-two. POV Jiraiya sc2; Tsunade sc5. |
| **67** | month 1, the 9th to the 13th, dawn. Five days. Seven scenes, ~8,500 words — **the interlude, light on purpose**. The missing rows; chapter twelve and the encoded protocol (*cf. II-12*); **the bath** — the crawl, the nosebleed, the Mystical Palm, the paragraph kiss, three seconds and tactical submersion, **the vow under water**; her log: *he tried to do something nice for me* (no suspicion — the fuse only grows); **row one**, the squid stall, laugh eighteen in the wild; the creed's pending-amendment note in book four; Monday dawn, the north fields, **Fukasaku present with the first answer**. POV Tsunade sc5. |
| **66** | month 1, the 2nd to the 9th. Eight days. Eight scenes, ~14,000 words. Naruto in the yard (*glad*); **the attachment is Tenzō**, effective Mon 6/1; Friday's three clean negatives; **the banked question** (where does an open article keep what it draws); Shizune's true sentences; the ferry law (*through and out*); the hat, 340; **the five-ryō win and the four seconds**; the Tarugi audit; the slab — *we'll wait*; **the IOU completed: PAID IN FULL, the seller confirms she was overpaid**; snow. POV Tsunade sc3, sc8; Tenzō sc4; Shizune sc5. |


### FIXED — the standing week (as of ch70–72; check before scheduling ANY scene)

| Slot | Owner |
|---|---|
| Mon/Tue/Wed/Fri mornings, six sharp | The field (Jiraiya) |
| Noons, every training day | **Hers. The through-and-out law: no items at the tray.** |
| Afternoons | The yard: Hime fabrication + spring research (Tenzō at eight feet) |
| **Thursdays, entire** | **Hers. No field, no yard, no board, NO ENTRIES.** |
| Fridays, four o'clock | Katsuyu's session, the quartermaster's yard |
| Tuesday evenings | **Naruto's apartment (standing, the chair)** |
| One Sunday a month | The family dinner, the second floor |

**Do not schedule work, reviews, tours, or meetings on a Thursday. Ever.
This has been violated once (ch72, caught by the author) and corrected.**

### FIXED — the price scale (established ch66, Tarugi market, 8/1)

Do not re-derive prices; check against these. A street snack runs ~10 ryō,
a working garment ~300, a sack of fuel ~200. "Multi-million ryō" therefore
means serious industry, not pocket money.

| Item | Price |
|---|---|
| Charcoal, per sack (winter, firm) | 220 |
| Felt hat, fair value (asked 900) | 340 |
| Squid skewer (ch67, east wall) | 11 |
| Winter pears | 11 each; 120/dozen |
| Dried gourd, bundle | 40 (marked down from 60) |
| Cups game, per go | 5 |
| Lake ferry, per crossing | 3 |
| Icha Icha ch9 plumbing repair (in-book) | 900 |

### FIXED — where Tsunade actually lives

**She sleeps at the tower**, four nights in five. Second floor. Ch48 sc1 is the
tower, not her house: she is at the window at five past eleven at night with a
border commission report, in her shirt with her hair down, and comes down the
**outside stair** when a guard says his name up it. Every ch48 header says *the
Hokage's tower*. **Sazare has never been inside her house before ch54**, and
that stands.

**The house is the one she has arranged not to be in.** Bare, made bed, nothing
on the drawers, one shelf, no photographs, and a glass of water by the bed with
three weeks of dust on it. She keeps it and pays for it and does not get rid of
it, and the reason is not stated and should not be.

**So the ch54 carry needs its reason and now has it** (sc7): the tower is where
she is *findable* — Shizune at the fifth hour, the guard change at the fourth,
the clerk with a key — and the house is the only building in the village where
nobody will open a door on her. He picks the correct place and it is the wrong
one, and he knows both.

**Do not later write her going home in the evening.** She goes upstairs.| 46 | month 6 through 29/7. **Six weeks — build time, the one span the model says must be real.** First note 28th night (~8/7); the test 11/7; Tsunade over the wall 29/7 |
| 42 | month 4 day 4 → month 5 day 6. **Five weeks, not a month-plus** — the new pacing floor. Landing mornings dated individually |

Other fixed dates: Ashida 19/1 · Miyuki 4/2 · Kiyomi 23/11 · Tarou 6/1 ·
Handa 2/3 · Emiko 12/10 · Kanzaki 30/6 · **Tsunade 2/8** (canon).

**Corrected once already:** the prose twice wrote his birthday as "the eleventh
of last month" against a world-bible date of day 8, and ch37–38 were sitting in
months 8–10 where the arithmetic could not close. Ashida's birthday was also set
three weeks out and then skipped past off-page.

**Corrected twice (ch74 planning pass):** ch72 sc9 and ch73 sc1 both occupied
the evening of Tuesday 10/3 with incompatible content — he is asleep after the
hair in one, awake at the desk until 23:20 in the other. Underneath it, ch72
sc8's build did not close: a Friday-night audit on 6/3 plus a two-week build
cannot deliver on the second Tuesday. **Repaired:** the chair is bought down
to four days on the curing article (ch72 sc8), so the 6/3 commission delivers
10/3 and ch72 keeps its curtain; and ch73 sc1 moves to **Wednesday 11/3**
("three days" to the assessment, not four). Month 3 runs Sun 1/3, so Tuesdays
are 3, 10, 17, 24 — **check a new scene against that before dating it.**

**The trap:** "nine years" is correct for the *trade and the knee* (both start at
~7) and wrong for *the family*, who were alive until 13y6m. Loss is measured in
**four years**, not nine. One line in ch35 was written wrong on this and has been
corrected.

---

## THE SPINE

The anchor is the **Konoha Crush**, which is also the start of the canonical
**2.5-year timeskip (30 months)**. Sazare is **16y7m** at the invasion.

| Chapter | Age | Months post-invasion | Content |
|---|---|---|---|
| 22 | 16y1m | −6 | Gate, Yamanaka, assessment, item six |
| 23 | 16y2–5m | −5 to −2 | Hospital: portering, the ruling, smoothing |
| 24 | 16y7m | −0 | Library, bed eleven, seeding, the street, eve of the finals |
| 25–27 | 16y7m | 0 | Invasion, the street, the audit, the terms |
| 28 | 16y8m | +1 | Acting Charge, rehab, Urushibara, Anko, Kakashi, the latch |
| 29 | 16y9m | +2 | The medical crystal trial (29 days + 5) |
| 30 | 16y10m | +3 | Ward Lead, Tsunade, the necklace, the terms in her language |
| 31 | 17y0m | +5 | Clearances, fundamentals, reproducible, the evaluation |
| 32 | 17y1m | +6 | Kakashi ends training, the compliment problem, Handa discharged |
| 33 | 17y3m | +8 | The asking, the digest, Yagura, a tenth of everything, two records |
| 34 | 17y4m | +9 | Mei, the scroll, the drink — **five weeks after ch33** |
| 35 | 17y5m | +10 | Surviving claimant, the corridor, the signature |
| 36 | 17y7m | +12 | First consignment, surplus arithmetic, Handa's shed |
| 37 | 17y10m → 18y1m | +14 to +18 | The book, gifts, the bells, where people look |
| 38 | 18y1m → 18y2m | +18 to +19 | Sakura, Yagi's ear, eleven birthdays, ch5 fails, **the letter** |
| 39 | 18y2m | +19 to +20 | The year they stopped, one invitation, Ashida's, six tables, **the hug** |
| 40 | 18y3m → 18y4m | +20 to +22 | **Urushibara dies.** The book returned, the next one taken, Hime, **Tsunade POV**, the hypothesis |

| 41 | 18y4m → 18y5m | +22 to +23 | Skill decay and recovery; **the material participates**; the seventh hand; **the birthday asked**; the trade paid; the letter not sent |

| 42 | 18y5m | +23 to +24 | Sakura and the manual; the marking scheme; ch5 paid on Handa; Kakashi and falling; **the governor**; ch4 reclassified *refused* |

**Position at end of ch42: month ~24 of 30. Six months remain.**

**Pacing floor set at ch42:** chapters now cost weeks, not months. Named
days inside a chapter (the fourth, the ninth, the sixteenth) rather than a
month label at the top. This is what makes the remaining queue fit.

**Retcon applied (see git history).** ch34 originally opened on an eleven-month
skip and ch33 closed on "still open eleven months later." Both were compressed
to **five weeks**, recovering ten months of runway. The in-story join is better
for it: he leaves the reparations query open intending to leave it for years,
and the normalisation declaration moves five weeks later. Ages swept across
ch34–37; he now turns eighteen inside ch37 rather than nineteen.

---

## WHAT REMAINS

**Twelve months of quiet, and everything outstanding has to fit inside it.**

- **The Kabuto approach** (beat 27b) begins in the remaining window and runs
  across the Act 4/5 boundary. It does not resolve inside Act 4.
- **The surplus** (30 kg, ~16 months at the ch36 rate) is **eight months in and
  will not complete before the operation eats the rest.** He loses the schedule
  he built himself, not the trip. Correct shape of loss for this book.
- **Naruto's return is the Act 4 curtain.** He met Sazare at sixteen in Wave and
  is the one person who ever asked *what was she like* (ch23). Unused, and the
  right ending.
- **Sword, Guren, and the Kiri trip are Act 5**, post-skip, into the Akatsuki
  era — where `chapter-plan.md` already has a Guren POV waiting.

**Do not spend the twelve months the way the first eighteen were spent.**

## LOOSE THREADS — open, with owners

| Thread | Opened | State |
|---|---|---|
| Reparations claim | ch35 | Filed. 2–3 yrs to determination. Kudō writes either way. |
| Hazama's four queries (grave) | ch36 | Sent. Burial register, clearance, survey, temples. None back. |
| The surplus (30 kg) | ch36 | ~8 months in of ~16. Will not complete — operation eats the rest. |
| The scroll — what he does with it | ch34 | **Deliberately undecided.** Do not resolve in advance. |
| Chapter four — Tsunade's gift | ch37 | **Reclassified ch42 sc7: not *unattempted* — *refused, five times, by me.*** Reason now written down: "an extravagance is a demand disguised" (p.61). He wants something back, knows it, will not act on it. Still open. |
| Chapter two — speech without object | ch37, ch51 | **PAID ch51 sc6** — four words in a gateway with nothing on the table, after he stopped constructing the better version. First time in his life. **ch42: 6/10 on a good morning.** Sakura's correction beats the book — the object is the other person. He has flagged himself that it is a habit with one person in it, not a capability. |
| Chapter five — the shoulder | ch37 | **PAID (ch42 sc3).** Handa, the shed, evening of the 16th, mid-sentence, reaching for a try square. Deliberate attempt forty minutes earlier was a disaster. |
| Chapter six — eleven birthdays | ch37 | Handa first. |
| Yagi's one pierced ear | ch37 | He intends to ask. Has not. |
| Ochiai Sumiko's name | ch37 | Learned. Four years late. |
| The two bells that do not ring | ch37 | Unexplained. He was stopped from drawing it. |
| The *gamble* kanji | ch37 | Noticed after two years. Evidence he cannot explain away. |
| Is it the mechanism or is it her | ch35, ch37 | Twice constructed, twice refused. Unresolved by design. |
| **Sakura** | — | **NOT YET ON PAGE. See below.** |
| Urushibara Gen | ch28–32 | **PAID (ch40 sc1).** Died 4th of the 2nd month. The open question — whether the last lesson was deliberate — is left at 8-in-10 and unresolvable. |
| Root | ch27, ch34 | Quiet. The quiet is itself the news. |
| Guren alive | ch27 | Confirmed as of 8 days pre-invasion. Untouched since. **Act 5.** |
| Sakura | ch42 | **Foregrounded.** Half-seven landing, the east stair, Ino and the eight who stopped asking. She and Sazare share the same unanswerable question from opposite ends (picked for the technique, or not). The third person. |
| The governor | ch42 | Ceiling as deepest course; every fragment inherits it; costs 31%. Signed into Kanzaki's ledger in his own hand. **Unwitting sword design.** The stray clause — *including if somebody takes it off my body* — left in the notebook as an open query. |
| The east stair | ch42 | Sakura's, unresolved. She has not used the short stair in eighteen months and has not told Ino. |
| Kakashi / physical training | ch28, ch42 | **Restarted after 15 months.** 51 seconds; genin 34; honest ceiling offered unasked at 41. Weekly. The diagnosis: he has never given up a position on purpose. |
| Kabuto approach | ch38+ | Letter received and reported (ch38 sc6). Reply written, **not sent** (ch41 sc6). Drawer disclosed to Ibiki unprompted (ch43 sc2) and slipped, six weeks late. **Controlled correspondence agreed:** three exchanges over ~9 months, first two true and unaltered. The fee — it will help him — stated in advance and accepted in writing. First letter goes 2nd Thursday of month 5. |
| Kurihara Saeko | ch43–45, ch50 | 20. Urushibara's student six years, **left two years ago**, was not at the funeral, found out off a form. Certifying the stones. Better at sealwork than him and says so. Holds the lacquered pin — thirty years too old for her, unexplained. Given the last winter **whole** (ch44 sc3), including the letter to the Land of Rivers. She left over a curriculum — she wanted the sealwork written down so it could be taught, he refused, she said something unforgivable in front of two people and he said worse. She works out mid-scene that the eleventh-month reorder was him doing what she asked, two years late, for somebody else, without telling her. **First crack ch44 sc6:** he answers a direct question with "no." Visit eleven ends without a named day. **ch45 sc5:** brings her own lens, softens two corrections, uses his given name once, books two months so she is only in the yard by contract. **The second certification (11–14/7) happened offstage** — thirty corrections in four days, all softened, and his log for it contains no person, plus a gap at four on the 13th he cannot recover. **ch50 sc3:** requisition 40-Q3/4 filed 4/9 with the personnel line completed rather than blank, for the first time in his life. No response as of 30/9. **The debt stands: eleven seconds, unpaid since 22/5.** |
| Urushibara — did he know | ch40, ch43 | Moved 8-in-10 → **9-in-10** on Kurihara's evidence (he only ever reordered a sequence to compress a syllabus). Deliberately held at one item of evidence. |
| The paper leak | ch43 | Marking approved **with public notice** — Tsunade's ruling. Notice signed by her, up Monday of month 5. Anything surviving the notice is by definition the serious version. |
| Ōkubo Hidetsugu | ch43 | 56, general surgery, 11 yrs. Left hand has a tremor with ~4 years in it. Volunteered as practitioner one. Wants a room where somebody is allowed to tell him he is wrong. |
| Tsunade's birthday | ch41 | 2/8. Given freely. He deliberately opened **no** design file — five months' notice refused on the grounds that a date is a deadline, not a requirement. Chapter four still unpaid. |
| The material-participates study | ch41 | Six practitioners, half silent. Tsunade will sign it if he gets the answer. Review board of eleven. 4–9 months. **ch42: Sakura volunteers for the silent half; routed via Yagi, not Tsunade, so it stays hers.** Deliberately unresolved past the timeskip. |
| The sentence he may not write | ch41 | "Outcome is partly the patient's will." Tsunade stopped it out loud. He formed it unprompted — the hypothesis-throwing failure mode, live and demonstrated. |

---

## SAKURA — the correction

**She has been absent from ch30–37 and should not have been.** She is Tsunade's
apprentice at that hospital for most of that stretch, and Tsunade discusses her
actual working life with him every Thursday. That is an authorial omission.

**The half that is not an omission:** it is entirely in character that he has
never spoken to her. He went two years without asking Ochiai's name. He does not
initiate; he treats people as terrain until somebody assigns him a role. Passing
the Hokage's apprentice in a corridor forty times across two years without
opening his mouth is precisely the chapter-three failure ch37 diagnosed.

**The fix is not to retro-write seven chapters.** Foreground her from ch38 with
the acknowledgement built in — he has seen her repeatedly for two years, never
spoken, and now knows why. That converts the omission into characterisation
instead of pretending it was planned.

**The thread that has been waiting since ch21:** Sakura is the one who gave him
the word. On a road, at sixteen, off the top of her head in nine seconds —
*tree-walking, it's the second control exercise* — the first name anybody ever
handed him for anything. And per beat 18 she is the safe echo of what pulled
Tsunade toward him: exceptional control she does not know is her best asset.
